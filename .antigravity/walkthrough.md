...
- Created unique index on `(tenant_id, portal_email, deleted_at)`
- Created fast lookup index on `portal_email`

#### Service Credentials  
- Renamed `username` → `service_username` for clarity
...
**Key Changes**:
- Renamed `Username` → `ServiceUsername` (mapped to `service_username` column)
...
Added new methods to `CustomerDatabasePort`:
...
- ✅ Separated portal and service credentials
- ✅ Secure encryption for service passwords
- ✅ Provisioning workflow tracking
- ✅ Customer portal session management
- ✅ Strong type safety with Go models
