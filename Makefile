...
.PHONY: build http message command model domain migration-postgres inbound-http inbound-http-fiber inbound-http-gin inbound-message-rabbitmq inbound-command outbound-database-postgres outbound-http outbound-message-rabbitmq outbound-cache-redis run generate-mocks
...
inbound-http-gin:
	...
	if ! grep -q "func (s \*adapter) $${PASCAL}()" "$$REGISTRY_FILE"; then \
		...
		echo "[INFO] $${PASCAL} method already exists in gin registry"; \
	fi; \
	REGISTRY_INTERFACE_FILE=internal/port/inbound/registry_http.go; \
	if grep -q "type HttpPort interface" "$$REGISTRY_INTERFACE_FILE"; then \
		if ! grep -q "$${PASCAL}() $${PASCAL}HttpPort" "$$REGISTRY_INTERFACE_FILE"; then \
...
run:
	...
	if [ -n "$$target" ]; then \
		...
		case "$$target" in \
			"model"|"domain"|"migration-postgres"|"inbound-http-fiber"|"inbound-http-gin"|"inbound-message-rabbitmq"|"inbound-command"|"outbound-database-postgres"|"outbound-http"|"outbound-message-rabbitmq"|"outbound-cache-redis") \
